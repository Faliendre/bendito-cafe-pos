...
import { BarChart, Package, ReceiptText, Sparkles } from 'lucide-react';

export function AdminSidebar({ active }: { active: 'dashboard' | 'products' | 'sales' }) {
    return (
        <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-ghost flex flex-col md:pt-8 md:pb-8 p-4 md:px-6 flex-shrink-0" style={{ background: 'var(--color-surface-container-low)' }}>
            ...
            <Link href="/" className="md:mt-auto flex items-center justify-center gap-2 font-bold p-3 md:p-4 rounded-2xl border-ghost border shadow-sm transition-transform active:scale-95 hover:bg-black/5 text-sm md:text-base relative overflow-hidden group" style={{ color: 'var(--color-primary)', background: 'var(--color-surface-container)' }}>
                <span className="relative z-10 hidden group-hover:block"><Sparkles size={18} className="inline mr-2 text-yellow-500 animate-pulse" />Easter Modo</span>
                <span className="relative z-10 group-hover:hidden">Volver al POS</span>
            </Link>
        </aside>
    );
}
