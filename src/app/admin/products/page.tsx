...
export default function AdminProducts() {
    ...
    // Updated modalData to include description
    const [modalData, setModalData] = useState<Partial<Product> & { description?: string }>({ 
        name: '', 
        price: 0, 
    ...
    async function saveProduct(e: React.FormEvent) {
        ...
        try {
            const payload = {
                ...
                description: modalData.description // Added description
            };

            if (modalData.id) {
                await supabase.from('products').update(payload).eq('id', modalData.id);
            } else {
                await supabase.from('products').insert(payload);
            }
            setShowModal(false);
            setModalData({ name: '', price: 0, category: 'Bebidas Calientes', variable_price: false, active: true, description: '' });
            showMessage("Producto Guardado", `El producto se ha guardado correctamente.`, "success");
            loadProducts();
        } catch (error) {
            console.error("Error saving product", error);
            showMessage("Error", "No se pudo guardar el producto.", "error");
        }
    }
        ...
        <div className="min-h-screen flex flex-col md:flex-row text-on-surface" style={{ background: 'var(--color-surface)' }}>
            ...
            <main className="flex-1 p-4 md:p-8 lg:p-12 overflow-y-auto">
                <header className="mb-6 md:mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                    <div>
                        <h2 className="font-display text-3xl md:text-4xl font-bold tracking-tight" style={{ color: 'var(--color-on-surface)' }}>Catálogo de Menú</h2>
                        <p className="opacity-70 mt-1 md:mt-2 text-base md:text-lg">Gestiona el menú digital y el sistema de ventas.</p>
                    </div>
                    <button
                        onClick={() => { setModalData({ name: '', price: 0, category: 'Bebidas Calientes', variable_price: false, active: true, description: '' }); setShowModal(true); }}
                    ...
                    <table className="w-full text-left border-collapse min-w-[600px]">
                        <thead>
                            <tr className="border-b border-ghost text-sm md:text-base" style={{ background: 'var(--color-surface-container-low)' }}>
                                ...
                                <th className="p-4 md:p-5 font-bold">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map(p => (
                                <tr key={p.id} className={`border-b border-ghost/50 transition-colors hover:bg-black/5 ${!p.active ? 'opacity-50' : ''}`}>
                                    ...
                                    <td className="p-5">
                                        <div className="font-bold text-lg">{p.name} {p.variable_price && <Tag size={14} className="inline ml-2 opacity-50" />}</div>
                                        <div className="text-xs opacity-50 font-medium truncate max-w-xs">{p.description}</div>
                                    </td>
                                    ...
                                    <td className="p-5 flex gap-3">
                                        ...
                                        <button
                                            ...
                                            className="font-bold px-4 rounded-xl border border-ghost hover:bg-black/10 transition-colors text-sm min-w-[100px]"
                                        >
                ...
                {showModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center glass-panel p-4 overflow-y-auto">
                        <div className="bg-white p-8 rounded-3xl max-w-lg w-full shadow-ambient relative" style={{ background: 'var(--color-surface-container-highest)' }}>
                            ...
                            <form onSubmit={saveProduct} className="space-y-4">
                                <div>
                                    ...
                                    <input required value={modalData.name} onChange={e => setModalData({ ...modalData, name: e.target.value })} className="w-full p-4 rounded-xl border border-ghost shadow-sm focus:outline-none" style={{ background: 'var(--color-surface-container-lowest)' }} placeholder="Ej: Espresso" />
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-bold opacity-70 mb-2">Descripción (Para el Menú Digital)</label>
                                    <textarea 
                                        value={modalData.description || ''} 
                                        onChange={e => setModalData({ ...modalData, description: e.target.value })} 
                                        className="w-full p-4 rounded-xl border border-ghost shadow-sm focus:outline-none min-h-[100px] resize-none" 
                                ...
                                <div className="flex items-center gap-4 p-4 border border-ghost rounded-xl shadow-sm" style={{ background: 'var(--color-surface-container-lowest)' }}>
                                    ...
                                    <label htmlFor="vprice" className="font-bold cursor-pointer">PRECIO VARIABLE<br /><span className="text-xs font-normal opacity-70 font-sans">Ideal para productos por peso o extras.</span></label>
                                </div>
